...
export default class DataService implements IDataService {
  ...
  public getTodos(sharePointApi: string, todoListName: string, hideFinishedTasks: boolean): ng.IPromise<ITodo[]> {
    ...
    let url: string = sharePointApi + 'web/lists/getbytitle(\'' + todoListName + '\')/items?$select=Id,Title,Status,Author/Title,Author/SipAddress,Created&$orderby=ID desc&$expand=Author';
    ...
    this.$http({
      ...
      for (let i: number = 0; i < result.data.value.length; i++) {
        ...
        todos.push({
          ...
          author: todo.Author.Title,
          created: new Date(todo.Created).toLocaleString(),
          pictureUrl: '/_layouts/15/userphoto.aspx?size=M&accountname=' + todo.Author.SipAddress
        });
      }
